...
def gerar_lista(motor, disj_motor, disj_geral, cabo, tipo_partida, res_data=None, pot_unit=None):
    lista = []
    if motor:
        motor_val, corrente, disj_motor_val, disj_geral_val = motor
...
# =====================================================
aba5 = st.tabs(["🎛️ Simulador"])[0]

with aba5:
    st.subheader("💨 Simulação de Pressostatos e Ventilador")
    
    # Nível atual de sujidade do filtro
    nivel = st.slider("Nível de Sujidade do Filtro", 0, 500, 0)
    
    # Limites de ativação/desativação dos pressostatos
    p1_ativar = st.number_input("Pressostato 1 - Ativar (nível)", min_value=0, max_value=500, value=250)
    p1_desativar = st.number_input("Pressostato 1 - Desativar (nível)", min_value=0, max_value=500, value=200)
    
    p2_ativar = st.number_input("Pressostato 2 - Ativar (nível)", min_value=0, max_value=500, value=400)
    p2_desativar = st.number_input("Pressostato 2 - Desativar (nível)", min_value=0, max_value=500, value=350)
    
    st.subheader("🔹 Status Atual")

    # Lógica de pressostatos com histerese
